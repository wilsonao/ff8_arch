...
| Junction locks (offsets 700-711: HP-J/Str-J/Vit-J/Mag-J/Spr-J/Spd-J/Eva-J/Hit-J/Elem-Atk-J/ST-Atk-J/Elem-Def-J/ST-Def-J) | progression | in pool only under `junction_locks` (F7, 2026-09-03). Party-wide: while an item is missing, its ability bits (Elem-Def-J also governs Elem-Defx2/x4, ST-Def-J the ST-Def-Jx2/x4) are cleared from ALL 16 GF records and any magic junctioned to that stat is zeroed in the char records (per-stat bytes `+0x5C..0x6E`, Hyne field order — CONFIRMED live 2026-09-03). On receipt the GFs' DEFAULT bits are re-asserted every tick (vanilla state, self-heals; restore domain = option-governed bits only). Luck-J excluded (Cactuar's signature). One random item precollected. Mastered checks gate on the items covering the GF's learn list; the 150 ladder tier keeps ≥150 uninterceptable abilities (154, test-asserted) |
| Command locks (offsets 720-723: Magic/GF/Draw/Item Command) | progression | in pool only under `command_locks` (F7). Same mechanism on ability bits 20-23 (in every GF's default mask and learn list): cleared from all GF records while locked + equipped command slots (`+0x50-0x52`) holding the id zeroed; restored as defaults on receipt. Attack/limits always remain; field-menu item use is untouched. v0.2.1: **Draw Command is precollected** (locking it gated ~240 draw-dependent checks — drought, not challenge); Magic/GF/Item are the pool items. Logic belt-and-braces: draw-dependent locations (`draw`/`world_draw` groups, `magic_drawn_once` stat checks) require **Draw Command**; the scanned ladder requires Draw + Magic (Scan is draw-in-battle, cast pre-clamp) — so even a plando'd-away precollect can't bury Draw Command behind a draw point. All draw-dependent checks also require ≥1 GF item under ANY options (a draw point needs a party member with the Draw command, which needs a junctioned GF) |
...
- **Single-instance guard (2026-09-03)**: only one client may drive the one FF8 process.
  Each attached client heartbeats a PID-stamped `FF8AP/client.lock`; a client that finds
  another's fresh (<6 s) heartbeat stands down completely — no reads, writes, or check
  sends — until it goes stale, then takes over. Two concurrent clients keep independent
  magic-cap ledgers and lock states and corrupt each other (observed live: client A
  granted Sleep x10, client B's cap still read 0 and repossessed it). Lock released on
  shutdown and on losing the game process.
